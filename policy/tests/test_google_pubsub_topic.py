# policy/tests/test_google_pubsub_topic.py
"""P4-7 structural tests for google_pubsub_topic Rego rules.
...
RULE_DIR = os.path.join(PROJECT_ROOT, "policy", "policies", "google_pubsub_topic")
...
class CmekRequiredTests(_BaseRuleTests, unittest.TestCase):
    RULE_FILE = "pubsub_topic_cmek_required.rego"

    def test_checks_kms_key_name(self):
        self.assertIn("kmsKeyName", self.contents)


class IamNoAllusersTests(_BaseRuleTests, unittest.TestCase):
    RULE_FILE = "pubsub_topic_iam_no_allusers.rego"
